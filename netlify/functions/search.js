...
function makeLinks(txt, segs) {
  ...
  return txt.replace(re, (m, ts, dt) => {
    const s = segs.find(x => x.timestamp === ts || x.date.includes(dt.trim()));
    return s ? `<a href="${s.url}" target="_blank" class="cite-link" title="Watch at ${ts}">([${ts} from ${dt}])</a>` : m;
  });
}
